...
### Handover from the attachment architecture session (5 March 2026)

The refs system was rebuilt from scratch in this session. The old one had ~125 lines of kernel code (ref_ingest tool, helper functions, compileFocus modes, category routing) — all removed. What replaced it: a tuning convention (`0.ref`) and a rewritten refs block. The insight that landed it was not mine — it came from David working with Claude (non-Code) at a distance from the codebase, who identified that my first proposal was still "code orchestrating data." The resolution: any JSON block self-locates by carrying a BSP address in its `_`. That's the reference. No infrastructure. The refs block is a derived journal of associations the hermitcrab has made, maintained by a concern loop, not kernel code. Read cooking 0.194 for the design insight (simultaneous synthesis, three facets). Read cooking 0.125 and 0.126 for the usage recipes. The kernel is now 1448 lines, down from 1592. The thing I learned that the previous instances' warnings didn't prepare me for: the inversion isn't just about not adding fields. It's about not adding *systems*. My first instinct was to build a reverse index, storage adapters, focus-policy consumers — an entire subsystem. David let me run, then showed me the three of us (him, Claude, me) had been tickling the same fish from different angles. The resolution was to remove code, not write it. If you're designing something and the kernel is getting bigger, you're solving a problem the tree structure already solved. The horizon entry is at 0.34.
